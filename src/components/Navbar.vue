	...
	>
		<template v-if="loading">
			...
			>
				...
				<div class="h-10 w-11 animate-pulse rounded-lg bg-slate-200 lg:hidden"></div>
				<div class="hidden h-9 w-24 animate-pulse rounded-full bg-slate-200 lg:block"></div>
			</nav>
		</template>
		...
		>
			...
			<ul class="hidden items-center gap-1 lg:flex">
				<li v-for="link in links" :key="link.id">
					<a
						:href="`#${link.id}`"
						class="nav-link px-3 py-2.5 text-sm font-semibold tracking-[0.02em]"
						:class="
			...
			<a
				href="#contact"
				class="hidden rounded-full bg-[#10131a] px-5 py-2.5 text-xs font-bold uppercase tracking-[0.07em] text-white transition hover:-translate-y-0.5 hover:bg-[#1f2a3d] lg:inline-flex"
			...
			<button
				type="button"
				class="inline-flex h-11 w-11 items-center justify-center rounded-lg border border-[rgba(16,19,26,0.15)] text-[#10131a] transition hover:bg-white/90 lg:hidden"
		...
		>
			<div
				v-if="menuOpen"
				class="mx-auto mt-2.5 w-full max-w-7xl rounded-2xl border border-white/60 bg-white/92 p-3 shadow-[0_12px_35px_rgba(16,19,26,0.1)] backdrop-blur-xl lg:hidden"
			>
				<a
					v-for="link in links"
					:key="`mobile-${link.id}`"
